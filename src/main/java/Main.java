...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // Определяем папку для сохранения картинок. Если её нет, то создаём.
        File folder = new File("nasaPictures");
        File file = new File(folder, fileName);
        if (!folder.exists() && !folder.mkdir()) {
            throw new IOException("Не удалось создать папку для сохранения картинок.");
        }

        // Сохранение файла
        try (FileOutputStream fos = new FileOutputStream(file)) {
            pictureResponse.getEntity().writeTo(fos);
        }
